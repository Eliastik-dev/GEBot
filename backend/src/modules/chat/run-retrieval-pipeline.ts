...
export async function runRetrievalPipeline(ctx: ChatPipelineBindings): Promise<void> {
  ...
  const { req, res, deps, startedAt, body, message, locale, profileFromMetadata, sessionId, geoConsentFromBody, geoCountryFromBody } = ctx;
    ...
    ctx.conversationFull = `${ctx.preAnalysisTranscript}\n${message}`;
    if (
    ...
    if (
      ...
      if (/\b(universel|plus\s+universel|alternative|compar)\b/i.test(message)) {
        ...
        ctx.extractedMeta.synonyms = [
          ...new Set([...ctx.extractedMeta.synonyms, "G110", "inhibiteur universel", "universel"]),
        ];
      }
    ...
    if (isPersonalDrinkwareOutOfCatalog(ctx.conversationFull)) {
      ...
      sseWriteWithSession(
        res,
        sessionId,
        { done: true, audience: ctx.audience, handoff: drinkwareHandoff, geoCountry: ctx.effectiveGeoCountry },
        "done",
      );
      ctx.completed = true;
    res.end();
    ...
    ctx.metadataForSearch = 
      ctx.effectiveTheme === "automobile" ? { ...ctx.extractedMeta, fluid: null } : ctx.extractedMeta;
    ...
    if (
      ctx.extractedMeta.needs_clarification &&
      ctx.diagnosticResult.clarification_message &&
      !ctx.hasCatalogCitation
    ) {
      ...
      const response = ctx.diagnosticResult.clarification_message;
      startSse(res);
      ...
      sseWriteWithSession(
        res,
        sessionId,
        { done: true, audience: ctx.audience, handoff: ctx.handoff, geoCountry: ctx.effectiveGeoCountry, messageId: ctx.assistantMsgId },
        "done",
      );
      ctx.completed = true;
    res.end();
    ...
    if (
      ...
      if (correctionProduct) {
        ...
        const cacheKeyCorrection = `${ANSWER_CACHE_VERSION}|${locale}|${ctx.audience}|${ctx.effectiveTheme ?? "none"}|fb:${ctx.feedbackCorrection.trainingQuery.toLowerCase()}|${correctionProduct.slug}`;
    ...
    if (
      ...
      if (directSheetProductEarly) {
        ...
        const resellerPromiseEarly =
          locale === "pl" ? Promise.resolve<Reseller[]>([]) : getCachedResellers().catch(() => []);
        const cacheKeyEarly = `${ANSWER_CACHE_VERSION}|${locale}|${ctx.audience}|${ctx.effectiveTheme ?? "none"}|${ctx.queryForRetrieval.toLowerCase()}`;
        await deliverDirectTechnicalSheetTurn({
          res,
          sessionId,
          locale,
    ...
    ctx.preFilters = ctx.applyThemeHardFilter
      ? {
          filters: [...ctx.baseFilters, ...ctx.themeFilters, ...ctx.policyFilters],
          condition: "and" as const,
        }
      : ctx.audience
        ? {
            filters: [...ctx.baseFilters, { key: "audience", value: ctx.audience, operator: "==" as const }, ...ctx.policyFilters],
            condition: "and" as const,
          }
        : {
    ...
    if (env.PRODUCT_KNOWLEDGE_ENABLED && ctx.catalogSize > 0) {
      if (ctx.factualProductMode) {
        ctx.pkResolvedProducts = filterProductKnowledgeByQueryContext(
          filterProductKnowledgeByPenalties(
        ...
        ctx.pkResolvedProducts = await injectFeedbackBoostProducts(
          ctx.pkResolvedProducts,
          ctx.feedbackHardBoosts,
          ctx.pkLocale,
        );
        ctx.pkProductSlugs = ctx.pkResolvedProducts.map((p) => p.slug);
        ctx.resolvedNodes = buildRetrieverNodesFromProductKnowledge(ctx.pkResolvedProducts, ctx.pkRenderContext);
        ctx.retrievalPath = "product_knowledge";
        ...
        if (ctx.pkResolvedProducts.length > 0) {
          ctx.pkProductSlugs = ctx.pkResolvedProducts.map((p) => p.slug);
          ctx.resolvedNodes = buildRetrieverNodesFromProductKnowledge(ctx.pkResolvedProducts, ctx.pkRenderContext);
          ctx.retrievalPath = "product_knowledge";
          ctx.retrievalCount = ctx.resolvedNodes.length;
          ...
          console.log("[/api/chat] product_knowledge_route", {
            ...
            slugs: ctx.pkProductSlugs,
            resolvedTags: ctx.pkRouteTags,
            productTags: ctx.pkResolvedProducts.flatMap((p) => p.use_case_tags),
            feedbackBoost: ctx.feedbackHardBoosts,
          });
          const debugNodes = ctx.pkResolvedProducts.map((p) => summarizeProductKnowledgeForDebug(p));
    ...
    if (ctx.retrievalPath === "vector_rag") {
      ...
      ctx.resolvedNodes = Array.isArray(retrievedNodes) ? retrievedNodes : [];
      ctx.retrievalCount = ctx.resolvedNodes.length;
      ...
      if ((ctx.retrievalCount < 3 || ctx.retrievalCount === 0) && ctx.effectiveTheme && ctx.applyThemeHardFilter) {
        ...
        if (Array.isArray(fallbackNodes) && fallbackNodes.length > ctx.retrievalCount) {
          ctx.resolvedNodes = fallbackNodes;
          ctx.retrievalCount = ctx.resolvedNodes.length;
        }
      }
      ...
      if (ctx.retrievalCount === 0 && ctx.audience) {
        ...
        ctx.resolvedNodes = Array.isArray(localeNodes) ? localeNodes : [];
        ctx.retrievalCount = ctx.resolvedNodes.length;
      }
      if (!ctx.allowFrenchStandards) {
        ctx.resolvedNodes = ctx.resolvedNodes.filter((item) => {
      ...
      if (
        ...
        !hasExhaustProductInNodes(ctx.resolvedNodes)
      ) {
        ...
        if (Array.isArray(supplementNodes) && supplementNodes.length > 0) {
          ctx.resolvedNodes = mergeRetrievalNodes(ctx.resolvedNodes, supplementNodes);
          ctx.retrievalCount = ctx.resolvedNodes.length;
        }
      }

      ctx.resolvedNodes = dynamicRerank(
        ctx.resolvedNodes,
        ctx.queryForRetrieval,
        ctx.searchQuery,
        ctx.extractedMeta,
        ctx.effectiveTheme,
        {
          ...
          explicitProductSlugs: [
            ...new Set([
              ...ctx.catalogCitation.products.map((p) => p.slug),
              ...ctx.pkResolvedProducts.map((p) => p.slug),
            ]),
          ],
        },
      );
      ctx.resolvedNodes = filterRetrievalNodesByFeedbackPenalties(
        ctx.resolvedNodes,
        ctx.feedbackCtx.penalizeSlugs,
        ctx.feedbackHardPenalties,
      );
      ...
      if (catalogAnchor.length > 0) {
        ...
        const pdfNodes = ctx.resolvedNodes.filter((node) => {
          const meta = (node as { node?: { metadata?: Record<string, unknown> } }).node?.metadata ?? {};
          return meta.type !== "product_knowledge";
        });
        ctx.resolvedNodes = [...catalogNodes, ...pdfNodes];
        ctx.retrievalPath = "product_knowledge";
        ctx.pkProductSlugs = catalogAnchor.map((p) => p.slug);
      ...
      if (ctx.preTechnicalFilterCount > 0) {
        const debugNodes = ctx.resolvedNodes.slice(0, 6).map((node) => summarizeNodeForDebug(node));
    ...
    if (ctx.informationalRetrievalQuestion || ctx.extractedMeta.intent === "general_technical") {
      ...
      if (ctx.faqContextNodes.length > 0) {
        console.log("[/api/chat] faq_context_prefetch", {
          sessionId,
          chunks: ctx.faqContextNodes.length,
        });
      }
    }
    ...
    if (
      ...
      if (citedProducts.length > 0) {
        ...
        ctx.resolvedNodes = mergeRetrievalNodes(citedNodes, ctx.resolvedNodes);
        ctx.pkResolvedProducts = [
          ...citedProducts,
          ...ctx.pkResolvedProducts.filter((p) => !citedProducts.some((c) => c.slug === p.slug)),
        ];
        ctx.pkProductSlugs = [...new Set([...citedProducts.map((p) => p.slug), ...ctx.pkProductSlugs])];
        ctx.retrievalPath = "product_knowledge";
    ...
    if (ctx.faqContextNodes.length > 0) {
      ctx.resolvedNodes = mergeRetrievalNodes(ctx.faqContextNodes, ctx.resolvedNodes);
      console.log("[/api/chat] faq_context_injected", {
        sessionId,
        chunks: ctx.faqContextNodes.length,
        totalNodes: ctx.resolvedNodes.length,
      });
    }
    ...
    ctx.comparisonEligible = 
      ctx.pkResolvedProducts.length >= 2 &&
      ctx.retrievalPath === "product_knowledge" &&
      !ctx.productFollowUp &&
      !ctx.hasCatalogCitation &&
      !isFactualProductQuestion(ctx.citationScanText);
...
${ctx.productFollowUp && ctx.priorRecommendedProduct ? `\nTYPE_QUESTION: product_follow_up � MODE 1 OBLIGATOIRE. Produit d�j� conseill� : ${ctx.priorRecommendedProduct}. R�ponds UNIQUEMENT � la nouvelle question en prose courte ; ne r�p�te PAS la fiche produit ni les liens FT/FDS.` : ""}
${isFactualProductQuestion(ctx.citationScanText) && ctx.hasCatalogCitation ? "\nTYPE_QUESTION: factual_product � MODE 1: r�pondez factuellement (oui/non, limites) depuis FT/FDS du produit identifi�. Ne pivotez pas vers un autre SKU." : ""}
${!ctx.productFollowUp && !isFactualProductQuestion(ctx.citationScanText) && !ctx.hasCatalogCitation && isInformationalProductQuestion(ctx.queryForRetrieval) ? "\nTYPE_QUESTION: informational_faq � MODE 1: r�ponds DIRECTEMENT � la question en prose naturelle avec les faits de la fiche." : ""}
${ctx.hasCatalogCitation && !isFactualProductQuestion(ctx.citationScanText) ? "\nTYPE_QUESTION: cited_product � MODE 2 sur le produit catalogue identifi� (profil/domaine ignor�s si la fiche correspond)." : ""}
${ctx.comparisonEligible ? `\nTYPE_QUESTION: open_recommendation_comparison � MODE 3 OBLIGATOIRE. ${ctx.pkResolvedProducts.length} produits catalogue r�cup�r�s : comparez-les (Option A/B${ctx.pkResolvedProducts.length >= 3 ? "/C" : ""}) avec Avantages et Limites issus des champs catalogue ; ne recommandez pas un seul SKU.` : ""}
