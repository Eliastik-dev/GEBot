...
export async function runRetrievalPipeline(ctx: ChatPipelineBindings): Promise<void> {
  ...
  const { req, res, deps, startedAt, body, message, locale, profileFromMetadata, sessionId, geoConsentFromBody, geoCountryFromBody } = ctx;
    ...
    ctx.explicitProductTargeted = 
      ...
      isExplicitProductTargeted([ctx.citationScanText, effectiveQuery: ctx.effectiveQuery, message, ctx.queryForRetrieval]);
    ...
    if (
      ...
      if (informationalQuestion) {
        ...
        faqPreMatch = await searchFaqKnowledgeMatch(deps.index, queryForRetrieval: ctx.queryForRetrieval, locale).catch(() => null);
    ...
    if (ctx.pdfSupplementSlugs.length > 0) {
      try {
        const pdfNodes = await retrievePdfChunksForSlugs(deps.index, pdfSupplementSlugs: ctx.pdfSupplementSlugs, pkLocale: ctx.pkLocale, {
    ...
    if (ctx.informationalRetrievalQuestion || ctx.extractedMeta.intent === "general_technical") {
      faqContextNodes = await retrieveFaqKnowledgeChunks(deps.index, queryForRetrieval: ctx.queryForRetrieval, locale, {
        topK: 6,
      }).catch(() => []);
    ...
    if (ctx.retrievalCount === 0) {
      const noContextReply = buildNoContextFallback(locale, queryForRetrieval: ctx.queryForRetrieval, ctx.fluid);
