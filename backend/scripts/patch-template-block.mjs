...
const block = `SEARCH_QUERY: \${ctx.searchQuery}
...
const start = text.indexOf("SEARCH_QUERY:");
const end = text.indexOf("Instruction finale:", start);
if (start === -1 || end === -1) throw new Error("block not found");
const endLine = text.indexOf("`;", end);
text = text.slice(0, start) + block + text.slice(endLine + 2);
fs.writeFileSync(fp, text);
console.log("template block patched");
